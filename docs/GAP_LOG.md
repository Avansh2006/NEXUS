...
| ID | Specification | Existing State | Gap / Resolution Required |
| :--- | :--- | :--- | :--- |
| **B1** | Live Incremental FIR Ingestion | Full re-analysis resets or recalculates the whole graph. | **Resolved (Pass):** Created `data/demo/incoming/NXS-007.json` introducing suspect Karan Bhati linking to syndicate hub `SYN-PHONE-001` and `Veyra Services`. Implemented incremental backend endpoints `POST /api/demo/incoming` (with sub-100ms latency telemetry and cross-case discovery) and `POST /api/demo/incoming/remove` (clean rollback). Added UI button ("Stream FIR NXS-007" / "Retract NXS-007"), live latency telemetry banner, Cytoscape pulse highlight, and Playwright E2E coverage. |
| **B2** | Scale Proof & Performance | System tested up to 121 records (~146 nodes, 313 edges). | **Resolved (Pass):** Created `scripts/generate_scale_data.py` generating realistic multi-cell synthetic networks (1k, 10k, 50k nodes/edges). Added $k$-sampling approximate betweenness in `intelligence/analysis.py` for graphs $>500$ nodes (achieving 6.6× to 10×+ speedup with bounded error). Added analysis `telemetry` payload and UI Centrality Mode badge in Tactical HUD. Implemented interactive Cluster Meta-Node View in Cytoscape collapsing Louvain communities into single aggregate nodes for 60 FPS rendering. Documented comprehensive empirical benchmarks in `docs/PERFORMANCE.md`. |
